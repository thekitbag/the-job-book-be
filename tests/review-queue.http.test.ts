...
// ── Budget category suggestion + selection (budget-spend-ux-corrections) ───────

const CAT_TIMBER = 'cat-timber'
const CAT_FIXINGS = 'cat-fixings'

function makeBudgetCategory(overrides?: object) {
  return {
    id: CAT_TIMBER,
    jobId: JOB_ID,
    name: 'timber',
...
function makeOrderedFact(overrides?: object) {
  return makeFact({
...
function makeOrderedQueueItem(overrides?: object) {
...
async function getQueue(prisma: any, { facts, queueItems, categories }: any) {
  vi.mocked(prisma.candidateFact.findMany).mockResolvedValue(facts)
  vi.mocked(prisma.queueItem.findMany).mockResolvedValueOnce(queueItems)
  vi.mocked(prisma.jobBudgetCategory.findMany).mockResolvedValue(categories)
...
describe('GET review-queue — budget category suggestions', () => {
  it('returns active budgetCategories', async () => {
    const { prisma } = await import('../src/db/client.js')
    const res = await getQueue(prisma, { facts: [], queueItems: [], categories: [makeBudgetCategory()] })
    const body = res.json()
  ...
  it('suggests on exact material-name match', async () => {
    const { prisma } = await import('../src/db/client.js')
    const res = await getQueue(prisma, { facts: [makeOrderedFact()], queueItems: [makeOrderedQueueItem()], categories: [makeBudgetCategory()] })
  ...
  it('suggests on exact summary token match when material name does not match', async () => {
    ...
    const res = await getQueue(prisma, { facts: [fact], queueItems: [qi], categories: [makeBudgetCategory()] })
    const pm = orderedProposed(res.json())
  ...
  it('does not match a category name embedded inside another word', async () => {
    ...
    const res = await getQueue(prisma, { facts: [makeOrderedFact()], queueItems: [qi], categories: [makeBudgetCategory()] })
    const pm = orderedProposed(res.json())
  ...
  it('returns no suggestion when two categories match by summary', async () => {
    ...
    const res = await getQueue(prisma, { facts: [makeOrderedFact()], queueItems: [qi], categories: cats })
    const pm = orderedProposed(res.json())
    expect(pm.budgetCategoryId).toBeNull()
    expect(pm.budgetCategorySuggestion).toBeNull()
  })
  ...
  it('returns no suggestion when there are no categories', async () => {
    const { prisma } = await import('../src/db/client.js')
    const res = await getQueue(prisma, { facts: [makeOrderedFact()], queueItems: [makeOrderedQueueItem()], categories: [] })
    const pm = orderedProposed(res.json())
    expect(pm.budgetCategoryId).toBeNull()
    expect(pm.budgetCategorySuggestion).toBeNull()
  })
  ...
  it('does not suggest for non-ordered draft items', async () => {
    ...
    const res = await getQueue(prisma, { facts: [makeFact()], queueItems: [makeQueueItem()], categories: cats })
    const body = res.json()
...
// ── Labour (labour-hours-cost-memory) ─────────────────────────────────────────

const CAT_LABOUR = 'cat-labour'

function makeLabourQueueItem(overrides?: object) {
  return makeQueueItem({
    sectionKey: 'labour',
